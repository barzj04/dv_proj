...
function normalizeStage(raw){
  ...
  if(/\bII\b|\b2\b/.test(v) || /\bIIB\b|\bIIA\b|\bII\b/.test(v)) return 'Stage II';
...
d3.csv("Esophageal_Dataset_Cleaned.csv", d => {
  ...
  return {
    ...
    lymph_examined: parsed.primary_pathology_number_of_lymphnodes_positive_by_he,
  ...
  box.innerHTML = 'Could not load <code>Esophageal_Dataset_Cleaned.csv</code>. ' +
...
const COLORS = {
  primary:'#1C6E8C', primaryDark:'#0F4C5C', primaryPale:'#DCEBEF',
  risk:'#C1440E', riskPale:'#F5DDD1', good:'#4C956C', goodPale:'#DCEEE2',
  gold:'#B8860B', ink:'#152238', inkSoft:'#4A5A6B',
  categorical:['#1C6E8C','#C1440E','#B8860B','#4C956C','#6C5B7B','#8E9AAF']
};
const tooltip = d3.select('#tooltip');
function showTip(html, evt){
  tooltip.style('display','block').html(html)
    .style('left',(evt.clientX+14)+'px').style('top',(evt.clientY+10)+'px');
}
function hideTip(){ tooltip.style('display','none'); }

const state = { gender:'', stage:'', histology:'', age:'', country:'', search:'', stageDrill:'', stageSubstage:'', stageBand:'' };
...
function applyFilters(records){
  return records.filter(r=>{
    ...
    if(state.search && !r.id.toLowerCase().includes(state.search.toLowerCase())) return false;
    return true;
  });
}

function uniqueSorted(arr){ return Array.from(new Set(arr)).filter(x=>x && x!=='Unknown').sort(); }
function fillSelect(id, values, key){
  const sel = d3.select(id);
  values.forEach(v=>{ sel.append('option').attr('value',v).text(v); });
  sel.on('change', function(){ state[key]=this.value; refreshAll(); });
}
function initializeDashboard(){
    ...
    ['Stage I','Stage II','Stage III','Stage IV'].forEach(v=> stageSelect.append('option').attr('value',v).text(v));
    ...
    d3.select('#stageDrillDown').on('click', ()=>{
      ...
      const subs = Array.from(new Set(DATA.records.filter(r=> r.stage===state.stageDrill).map(r=> r.stage_raw))).filter(x=>x);
      state.stageSubstage = subs.length? subs[0] : '';
      d3.select('#f-stage').property('value', state.stageDrill);
      refreshAll();
    });
    d3.select('#stageDrillUp').on('click', ()=>{
      if(state.stageSubstage){ state.stageSubstage = ''; }
      else if(state.stageDrill){ state.stageDrill = ''; state.stage = ''; d3.select('#f-stage').property('value',''); }
      refreshAll();
    });
    d3.select('#stageDrillBack').on('click', ()=>{
      state.stage=''; state.stageDrill=''; state.stageSubstage=''; d3.select('#f-stage').property('value',''); refreshAll();
    });
}

d3.select('#patientSearch').on('input', function(){ state.search=this.value; refreshAll(); });
d3.select('#resetBtn').on('click', ()=>{
  Object.keys(state).forEach(k=> state[k]='');
  ['#f-gender','#f-stage','#f-histology','#f-age','#f-country'].forEach(id=> d3.select(id).property('value',''));
...
function syncIconButtons(){
  ...
  d3.select('.icon-btn[data-key="gender"][data-value=""]').classed('active', !state.gender);
  d3.select('.icon-btn[data-key="stageBand"][data-value=""]').classed('active', !state.stageBand);
}
syncIconButtons();
...
d3.select('#dataNotesToggle').on('click', function(){
  ...
  this.textContent = open ? 'ℹ  Data Cleaning & Assumptions' : '✕  Hide Data Notes';
});

function renderActiveChips(){
  const chips = Object.entries(state).filter(([k,v])=>v && k!=='search');
  const el = d3.select('#activeFilters').selectAll('.chip').data(chips, d=>d[0]);
  el.exit().remove();
  const enter = el.enter().append('div').attr('class','chip');
  enter.merge(el).html(d=>`${d[1]} &times;`)
    .on('click', (evt,d)=>{
      state[d[0]]='';
...
const kpiDefs = [
  {key:'total_patients', label:'Total Patients', unit:'', color:COLORS.primary, fmt:d3.format(',')},
  {key:'mortality_rate', label:'Mortality Rate', unit:'%', color:COLORS.risk, fmt:d3.format('.1f')},
  {key:'avg_age_dx', label:'Avg. Age at Diagnosis', unit:'yrs', color:COLORS.gold, fmt:d3.format('.1f')},
...
function barChart(svgSel, data, opts){
  const {xKey, yKey, stateKey, color, height, angle, isStage, isSubstage, colorKey} = opts || {};
  ...
  const w = width-margin.left-margin.right, h=H-margin.top-margin.bottom;
  const g = svg.append('g').attr('transform',`translate(${margin.left},${margin.top})`);
  const x = d3.scaleBand().domain(data.map(d=>d[xKey])).range([0,w]).padding(0.28);
  const y = d3.scaleLinear().domain([0, d3.max(data,d=>d[yKey])*1.15||1]).range([h,0]);
  g.append('g').attr('class','axis').attr('transform',`translate(0,${h})`).call(d3.axisBottom(x).tickSizeOuter(0))
    .selectAll('text').attr('transform', angle? 'rotate(-30)':null).style('text-anchor', angle? 'end':'middle');
  g.append('g').attr('class','axis').call(d3.axisLeft(y).ticks(4));
  if(!data || data.length===0 || d3.max(data,d=>d[yKey])<=0){
    svg.append('text').attr('x', width/2).attr('y', H/2).attr('text-anchor','middle')
      .attr('fill',COLORS.inkSoft).style('font-size','12px').text('No data available');
    return;
  }
  g.selectAll('.bar').data(data).enter().append('rect')
    .attr('class','bar')
    .classed('dim', d=> stateKey && state[stateKey] && state[stateKey]!==d[xKey])
    .attr('x', d=>x(d[xKey]))
    ...
    .attr('rx',3)
    .on('mousemove', (evt,d)=> showTip(`<b>${d[xKey]}</b><br>${d[yKey].toLocaleString()} cases`, evt))
...
function donutChart(svgSel, data, opts){
  ...
  svg.selectAll('*').remove();
  const bbox = svg.node().getBoundingClientRect();
  const width = bbox.width || 300; const H = height||240;
  svg.attr('viewBox',`0 0 ${width} ${H}`);
  const radius = Math.min(width*0.42, H*0.42);
  const g = svg.append('g').attr('transform',`translate(${width*0.32},${H/2})`);
  const color = d3.scaleOrdinal().domain(data.map(d=>d[labelKey])).range(COLORS.categorical);
  const pie = d3.pie().value(d=>d[valueKey]).sort(null);
  const arc = d3.arc().innerRadius(radius*0.55).outerRadius(radius);
  const total = d3.sum(data, d=>d[valueKey]);
  g.selectAll('path').data(pie(data)).enter().append('path')
    .attr('d',arc).attr('fill', d=>color(d.data[labelKey]))
    .attr('class','bar')
    .classed('dim', d=> stateKey && state[stateKey] && state[stateKey]!==d.data[labelKey])
    .on('mousemove',(evt,d)=> showTip(`<b>${d.data[labelKey]}</b><br>${d.data[valueKey].toLocaleString()} (${(d.data[valueKey]/total*100).toFixed(1)}%)`, evt))
    .on('mouseleave', hideTip)
    .on('click', (evt,d)=>{
      if(!stateKey) return;
      state[stateKey] = state[stateKey]===d.data[labelKey] ? '' : d.data[labelKey];
      d3.select('#f-'+stateKey).property('value', state[stateKey]);
      refreshAll();
    });
...
function countBy(recs, key, order){
  const m = d3.rollup(recs, v=>v.length, d=>d[key]);
...
function riskMultiples(recs){
  ...
  container.html('');
  const available = factorsMeta.filter(fm=> recs.some(r=>r[fm.field] && r[fm.field] !== 'Unknown'));
  if(available.length===0){
    container.append('div').style('height','100%').style('display','flex').style('align-items','center').style('justify-content','center').style('color',COLORS.inkSoft).style('font-size','13px').text('Risk factor charts are unavailable for this dataset.');
    return;
  }
  available.forEach(fm=>{
    const groups = d3.rollup(recs.filter(r=>r[fm.field] && r[fm.field] !== 'Unknown'), v=>({ n:v.length, dead: v.filter(r=>r.vital==='Dead').length }), d=>d[fm.field]);
    const data = Array.from(groups, ([k,v])=>({cat:k, n:v.n, pct: v.n? v.dead/v.n*100:0})).filter(d=>d.n>=8);
    const card = container.append('div').attr('class','risk-card').style('width','100%');
    card.append('div').attr('class','card-title').text(fm.title);
    if(data.length===0){
      card.append('div').style('font-size','12px').style('color',COLORS.inkSoft).style('text-align','center').text('Not enough data');
      return;
    }
    const vbW = 450, vbH = 220, margin={l:20,r:10,t:10,b:42};
    const svg = card.append('svg').attr('viewBox',`0 0 ${vbW} ${vbH}`).attr('width','100%').attr('preserveAspectRatio','xMidYMid meet');
    const w = vbW - margin.l - margin.r, h = vbH - margin.t - margin.b;
    const g = svg.append('g').attr('transform',`translate(${margin.l},${margin.t})`);
    const x = d3.scaleBand().domain(data.map(d=>d.cat)).range([0,w]).padding(0.35);
    const y = d3.scaleLinear().domain([0, Math.max(60, d3.max(data,d=>d.pct)||0)]).range([h,0]);
    g.append('g').attr('transform',`translate(0,${h})`).call(d3.axisBottom(x).tickSizeOuter(0)).selectAll('text').style('font-size','9px').attr('transform','rotate(-20)').style('text-anchor','end').attr('fill',COLORS.inkSoft);
    g.append('g').call(d3.axisLeft(y).ticks(3)).selectAll('text').style('font-size','9px');
    ...
    g.selectAll('circle').data(data).enter().append('circle')
      ...
      .attr('cy', d=> y(d.pct))
      .attr('r', d=> r(d.n))
      .attr('fill', COLORS.risk).attr('fill-opacity',0.85).attr('stroke',COLORS.risk)
      .on('mousemove',(evt,d)=> showTip(`<b>${fm.title}: ${d.cat}</b><br>Mortality: ${d.pct.toFixed(1)}%<br>n = ${d.n}`, evt))
      .on('mouseleave', hideTip);
  });
}
...
function lineChart(svgSel, data, opts){
  ...
  const bbox = svg.node().getBoundingClientRect();
  const width = bbox.width||500, H=height||230;
  svg.attr('viewBox',`0 0 ${width} ${H}`);
  if(!data || data.length===0){
    svg.append('text').attr('x', width/2).attr('y', H/2).attr('text-anchor','middle').attr('fill',COLORS.inkSoft).style('font-size','12px').text('No data available');
    return;
  }
  const margin={top:14,right:20,bottom:32,left:44};
  const w=width-margin.left-margin.right, h=H-margin.top-margin.bottom;
  const g = svg.append('g').attr('transform',`translate(${margin.left},${margin.top})`);
  const x = d3.scalePoint().domain(data.map(d=>d[xKey])).range([0,w]).padding(0.5);
  const y = d3.scaleLinear().domain([0, (d3.max(data,d=>d[yKey]) || 1)*1.2]).range([h,0]);
  ...
  g.selectAll('circle').data(data).enter().append('circle')
    .attr('cx',d=>x(d[xKey])).attr('cy',d=>y(d[yKey])).attr('r',4.5).attr('fill',color)
    .on('mousemove',(evt,d)=> showTip(`<b>${d[xKey]}</b><br>${(yFmt?yFmt(d[yKey]):d[yKey])}`, evt))
    .on('mouseleave', hideTip);
}
...
function renderParcoords(recs){
  ...
  Plotly.newPlot('chartParcoords', [{
    type: 'parcoords',
    line: {
      color: vitalColor, 
      colorscale: [[0, '#4C956C'], [1, '#C1440E']], 
      showscale: true,
      colorbar: { title: 'Vital', tickvals: [0, 1], ticktext: ['Alive', 'Dead'], len: 0.6 }
    },
...
function renderTrellis(recs){
  ...
  container.selectAll('*').remove();
  const collection = recs.filter(r=>r.pack_years!=null && r.lymph_examined!=null);
  if(collection.length===0){
    container.append('div').style('height','100%').style('display','flex').style('align-items','center').style('justify-content','center').style('color',COLORS.inkSoft).style('font-size','13px').text('Trellis plot not available for this dataset.');
    return;
  }
  ...
  stageData.forEach(dataset=>{
    ...
    g.append('g').call(d3.axisLeft(y).ticks(4));
    g.append('text').attr('x',0).attr('y',-8).attr('fill',COLORS.ink).style('font-size','11px').style('font-weight','700').text(dataset.stage);
    g.selectAll('circle').data(dataset.values).enter().append('circle')
      .attr('cx',d=>x(d.pack_years))
      .attr('cy',d=>y(d.lymph_examined))
      .attr('r',4)
      .attr('fill',d=> d.vital==='Dead'?COLORS.risk:COLORS.good)
      .attr('fill-opacity',0.75)
      .on('mousemove',(evt,d)=> showTip(`<b>${d.id}</b><br>Age: ${d.age ?? '—'}<br>Pack-years: ${d.pack_years}<br>Lymph nodes+: ${d.lymph_examined}<br>Vital: ${d.vital}`, evt))
      .on('mouseleave', hideTip);
    g.append('text').attr('x',w).attr('y',h+24).attr('text-anchor','end').attr('fill',COLORS.inkSoft).style('font-size','10px').text('Pack-years');
    g.append('text').attr('x',-38).attr('y',h/2).attr('text-anchor','middle').attr('transform',`rotate(-90,-38,${h/2})`).attr('fill',COLORS.inkSoft).style('font-size','10px').text('Lymph nodes+');
  });
}

function renderInsights(recs){
  const stage34 = recs.filter(r=>r.stage==='Stage III'||r.stage==='Stage IV');
  const stage34MortRate = stage34.length? (stage34.filter(r=>r.vital==='Dead').length/stage34.length*100):0;
  const early = recs.filter(r=>r.stage==='Stage I'||r.stage==='Stage II');
  const earlyMortRate = early.length? (early.filter(r=>r.vital==='Dead').length/early.length*100):0;
  const topCountry = Array.from(d3.rollup(recs, v=>v.length, d=>d.country)).sort((a,b)=>b[1]-a[1])[0];
  const insights = [
    {cls:'risk', title:'High-Risk Patient Group', stat:`${stage34MortRate.toFixed(0)}%`,
    ...
    {cls:'', title:'Regional Case Concentration', stat: topCountry? topCountry[0]:'—',
    ...
    {cls:'good', title:'Pathology Yield Indicator', stat:`${(d3.mean(recs.map(r=>r.lymph_examined).filter(x=>x!=null))||0).toFixed(1)}`,
  ...
  const grid = d3.select('#insightGrid').selectAll('.insight-card').data(insights);
  const enter = grid.enter().append('div').attr('class', d=>'insight-card '+d.cls);
  enter.append('h4');
...
function renderTable(recs){
  const title = state.stageSubstage ? `Patients — ${state.stageSubstage}` :
                ...
                state.stage ? `Patients — ${state.stage}` :
                (state.histology? `Patients — ${state.histology}` : 'All Patients');
  document.getElementById('drillTitle').textContent = title;
  document.getElementById('rowCount').textContent = recs.length.toLocaleString();
  ...
  merged.select('.stage').text(d=> (state.stageDrill && d.stage_raw) ? d.stage_raw : d.stage);
  merged.select('.histology').text(d=> d.histology==='Squamous Cell Carcinoma'?'SCC':(d.histology==='Adenocarcinoma'?'Adeno':d.histology));
...
function refreshAll(){
  ...
  barChart('#chartAge', countBy(recs,'age_group', ['<50','50-59','60-69','70-79','80+']).map(d=>({group:d.key,count:d.count})),
    {xKey:'group', yKey:'count', stateKey:'age', color:COLORS.gold, height:220});
  donutChart('#chartVital', countBy(recs,'vital').map(d=>({status:d.key,count:d.count})),
    {labelKey:'status', valueKey:'count', height:220});
  const countryTop = countBy(recs,'country').map(d=>({country:d.key,count:d.count})).sort((a,b)=>b.count-a.count).slice(0,7);
  barChart('#chartCountry', countryTop, {xKey:'country', yKey:'count', stateKey:'country', color:COLORS.primaryDark, height:220, angle:true});
  ...
  const stageOpts = {
    ...
    color:COLORS.primary, height:260, isStage:true, isSubstage: Boolean(state.stageDrill)
  };
  if(state.stageDrill){
    const stageRecs = recs.filter(r=> r.stage===state.stageDrill);
    const topCount = stageRecs.length;
    const m = d3.rollup(stageRecs, v=>v.length, d=> (d.stage_raw || '').trim() || state.stageDrill );
    const entries = Array.from(m, ([k,v])=>{
      const mm = String(k).toUpperCase().match(/^STAGE\s*(I{1,3}|IV)([A-Z]*)/i);
    ...
    stageData = countBy(recs,'stage', ['Stage I','Stage II','Stage III','Stage IV','Unknown']).map(d=>({
      stage:d.key, count:d.count, barColor: d.key==='Unknown' ? '#B9C2C0' : COLORS.primary
    }));
  }
  ...
  const resourceData = stageOrder.map(s=>{
    ...
    return {stage:s, count: sub.length? d3.mean(sub.map(r=>r.lymph_examined)):0};
  });
  barChart('#chartResource', resourceData, {xKey:'stage', yKey:'count', color:COLORS.primaryDark, height:230});
  ...
  if(!state.stage && !state.stageDrill && !state.stageSubstage){
    btnDown.style('display','none'); btnUp.style('display','none'); btnBack.style('display','none');
  } else if(state.stage && !state.stageDrill){
    btnDown.style('display','inline-block'); btnUp.style('display','none'); btnBack.style('display','inline-block');
  } else if(state.stageDrill && !state.stageSubstage){
    btnDown.style('display','none'); btnUp.style('display','inline-block'); btnBack.style('display','inline-block');
...
d3.selectAll('#viewToggle button').on('click', function(){
  ...
  setTimeout(()=>{ if(DATA.records.length) refreshAll(); }, 60); 
});

window.addEventListener('resize', ()=> { if(DATA.records.length) refreshAll(); });
